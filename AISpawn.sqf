MySpawnAI = {
	...
	while {_hasCargo} do {
		...
		// TODO* set based on encounter type? (e.g. cautious if heard shots)
	...
	[_myplane, _mylastPlanePos] call createWaypoint;
};
...
EndTrGenerator = {
	...
	if (DayNightCycle>0) then {
		call EnableDayNightCicle;
	};
};
...
switch (BRSEWeather) do {
  ...
  case 5: {IsWeatherVariable = true;};
	default {hint "ERROR - WRONG BRSEWeather!"};
};

// TODO* use actual times, not skipTime
switch (BRSETime) do {
  case 0: {};
  case 1: {skipTime 12;};
	case 2: {skipTime 16;};
  ...
  default {hint "ERROR - WRONG BRSETime!"};
};
