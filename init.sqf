...
updateUI = {
	if (EnableCustomHudMode > 0) then {
		...
		// TODO* call updateUITime from here?
...
updateUITime = {
	...
	_display = uiNamespace getVariable "hudScrDisplay";
	_textZoneTime = _display displayCtrl 1911;
	_minutes = floor (ZoneExpirationTime / 60);
	_seconds = ZoneExpirationTime - (_minutes * 60);
	_extra = "";
	if (_seconds < 10) then { _extra = "0"; };
	_textZoneTime ctrlSetText format ["%1:%2%3", _minutes, _extra, _seconds];
};
...
// Generate notifications about zones and update UI time
notifyGenerator = {
...
triggerLogicCreator = {
	0 = [] spawn {
		_BigCircleS = AreasArray select (roundCounter - 1); // Previous zone
		_SmallCircleS = AreasArray select roundCounter; // New zone
		_LasPosPossibleRad = _BigCircleS - _SmallCircleS; // Last radius
...

		{
			...
			// TODO* include player in this loop if option to debug enabled (so you don't have to play)
			if ((_x != player) && ((side _x) == sideEnemy)) then {
				...
				[_x] allowGetIn false;
				_grp setBehaviour "AWARE";
				if (_dist < _SmallCircleS) then { // If within circle
					...
					_cargAr = nearestObjects [position _x, ["car"], _carFindRad]; // refine the radius on small points especially
